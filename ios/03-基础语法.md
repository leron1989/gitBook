...
![屏幕快照 2019-01-03 下午1.04.06](./img/屏幕快照 2019-01-03 下午1.04.06.png)



![屏幕快照 2019-01-03 下午1.06.51](./img/屏幕快照 2019-01-03 下午1.06.51.png)

![屏幕快照 2019-01-03 下午1.06.51](./img/屏幕快照 2019-01-03 下午1.06.51.png)

程序时函数的集合

2. 局部变量、帧（frame）、栈（stack）

局部变量保存在函数的帧中，函数执行完，帧销毁

程序中使用栈来描述帧在内存中的存储地点，执行函数时，函数的帧会在栈的顶部被创建出来。函数结束，帧退出栈。

3. 全局变量，静态变量

   全局变量可以就可以作用于该程序的所有的源文件。其他不包含全局变量定义的源文件需要用extern 关键字再次声明这个全局变量，因此全局变量容易造成混淆

   静态变量它作用于定义它的文件里，不能作用到其它文件里，即被static关键字修饰过的变量具有文件作用域。这样即使两个不同的源文件都定义了相同名字的静态全局变量，它们也是不同的变量。

   无论全局变量还是静态变量都可以在创建时赋值
...
4. 数字
   ...
   * 整数分**<u>有符号</u>**和**<u>无符号</u>**两种类型

   * 整数有8位、16位、32位、64位，8位无符号可保存0～255数据，有符号需一位做符号位
     $$
     2^8=256
     $$
